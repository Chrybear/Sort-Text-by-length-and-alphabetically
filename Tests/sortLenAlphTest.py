...
class sortedTesting(unittest.TestCase):
    ...
    def testAscending(self):
        ...
        if os.path.isfile('sort me sorted A.txt') and os.path.isfile('Sort Me.txt'):
            ...
            self.assertFalse(True, 'Error: testing files not found. To run tests, the files (sort me sorted A.txt) and '
                                   '(sort me sorted D.txt) and (Sort Me.txt) must be in the same folder as '
                                  'sortLenAlphTest.py')

    # Test if text was sorted in descending order correctly
    def testDescending(self):
        # Need to first make sure the files with which we test with exist
        if os.path.isfile('sort me sorted D.txt') and os.path.isfile('Sort Me.txt'):
            # Get the sorted text file as an array to compare with
